...
%%
% compute firing rate over entire recording
% use wide windows for computational time
[spikes,waveforms,mua,thresh] = detectSpikes(data(:,1:240*fs),threshStds);

%%
% figure; hold on
% plot(data(3,1:240*fs));
% plot(-thresh(3)*ones(1,240*fs));  
% xlim([1 240*fs]);
% yl=ylim;
% stem(yl(1)*mua(3,:));
...
%%
binWidthMs=100;
[psth,binCenters] = computePsth(mua(3,:).',binWidthMs,fs);
...
secsElapsed = timeStr2Sample(blockStartTimesStr{1},recordStartTimeStr,fs);
plot([secsElapsed secsElapsed],[yl(1) yl(2)]);

secsElapsed = timeStr2Sample(blockStartTimesStr{2},recordStartTimeStr,fs);
plot([secsElapsed secsElapsed],[yl(1) yl(2)]);

secsElapsed = timeStr2Sample(blockStartTimesStr{3},recordStartTimeStr,fs);
plot([secsElapsed secsElapsed],[yl(1) yl(2)]);
% %%
